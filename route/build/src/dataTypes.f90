...
implicit none
 ...
 type, public :: runoff
   ...
   real(dp)                 , allocatable  :: subqsim(:)       ! subsurface runoff (HM_HRU) at one time step (size: nSpace(1))
   real(dp)                 , allocatable  :: subqsim2D(:,:)   ! subsurface (x,y) at one time step (size: /nSpace(1), nSpace(2)/)
   ...
   real(dp)                 , allocatable  :: basinSubRunoff(:)! remapped river network catchment subsurface runoff (size: number of nHRU)
 end type runoff
