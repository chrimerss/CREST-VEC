...
!  write(*,"(A,1PG15.7,A)") '  elapsed-time [runoff_input/read] = ', elapsedTime, ' s'
  ...
  if (is_remap) then ! remap LSM simulated runoff to the HRUs in the river network
   ...
   call remap_runoff(runoff_data, remap_data, runoff_data%basinRunoff, runoff_data%basinSubRunoff, ierr, cmessage)
   if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif

  else ! runoff is already remapped to river network HRUs

   call sort_runoff(runoff_data, runoff_data%basinRunoff, runoff_data%basinSubRunoff, ierr, cmessage)
   if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
