...
contains
  ...
  REAL(DP), INTENT(IN)                   :: subfshape      ! shapef parameter in gamma distribution
  REAL(DP), INTENT(IN)                   :: subtscale      ! time scale parameter  
  ! output
  ...
  if (doesSubSurfRoute==1) then
    ...
    if (.not.allocated(FRAC_FUTURE_SUB)) then
      ...
      if(ierr/=0)then; message=trim(message)//'unable to allocate space for the subsurface time delay histogram'; return; endif
    endif
