...
contains
 ...
 ! allocate space for subsurface runoff
 if (doesSubSurfRoute==1) then
 allocate(runoff_data_in%subqSim(runoff_data_in%nSpace(1)), stat=ierr)
 endif
 if(ierr/=0)then; message=trim(message)//'problem allocating runoff_data_in%qsim'; return; endif
 ...
 if (doesSubSurfRoute==1) then
 allocate(runoff_data_in%subqSim2D(runoff_data_in%nSpace(2), runoff_data_in%nSpace(2)), stat=ierr)
 endif
 if(ierr/=0)then; message=trim(message)//'problem allocating qsim'; return; endif
