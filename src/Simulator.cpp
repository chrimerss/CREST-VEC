...
bool Simulator::InitializeSimu(TaskConfigSection *task) {
  ...
  if (!wbModel->IsLumped()) {
    ...
    currentSQ.resize(lumpedNodes.size());
...
void Simulator::SimulateDistributed(bool trackPeaks) {
  ...
  runoffGrid.resize(currentFF.size());
  // NORMAL_LOGF("%s\n", "Got here!7");
  for (size_t i = 0; i < currentFF.size(); i++) {
    count2[i] = 0.0;
    rpGrid[i] = 0.0;
...
// NORMAL_LOGF("%s\n", "Got here!9");
    ...
    // NORMAL_LOGF("%s\n", "Got here!10");
    // NORMAL_LOGF("%s: %f", "Current precipitation", &currentPrecip[tsIndex]);
...
#endif
#endif
      rModel->Route(stepHoursReal, &currentFF, &currentSF, &currentQ, &currentSQ);
#if _OPENMP
#ifndef _WIN32
    ...
    if (warmEndTime <= currentTime) {
      ...
      if ((griddedOutputs & OG_SQ) == OG_SQ) {
        sprintf(buffer, "%s/sq.%s.%s.tif", outputPath,
                currentTimeTextOutput.GetName(), wbModel->GetName());
        gridWriter.WriteGrid(&nodes, &currentSQ, buffer, false);
      }      
      ...
      if (outputRP && ((griddedOutputs & OG_RUNOFF) == OG_RUNOFF)) {
...
float Simulator::SimulateForCali(float *testParams) {
  ...
  std::vector<float> currentFFCali, currentSFCali, currentQCali, currentSQCali, simQCali,
  ...
  currentSQCali.resize(currentFF.size());
